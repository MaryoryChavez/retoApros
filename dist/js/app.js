...
function catalogo(){
    //----------------------------Animacion de catalogo paneles!
    var $panel, $contenido_panel;
    $catalogo_menu = [$("#mini_galeria li:nth-child(1) a"), 
                      $("#mini_galeria li:nth-child(2) a"), 
                      $("#mini_galeria li:nth-child(3) a"), 
                      $("#mini_galeria li:nth-child(4) a"), 
                      $("#mini_galeria li:nth-child(5) a")];

    for (var i = 0; i < 5; i++) {
        $catalogo_menu[i].click(function(){
            $panel = $(this).attr('href');
            $contenido_panel = $($panel+">.row");
             Foundation.Motion.animateIn($contenido_panel, 'mui-slide');
        })
    }
}
        ...
        (function (i, mapMarker) {
            ...
            google.maps.event.addListener(mapMarker, "mouseover", function () {
                ...
                geocoder.geocode({
                    ...
                    if (status === 'OK') {
                        if (results[1]) {
                            document.getElementById('cajaDireccion').innerHTML = 
                                '<div id="direccion">'+'<img src="images/icon-location.svg" alt="">'+results[1].formatted_address +'</div>';
                        } else {
